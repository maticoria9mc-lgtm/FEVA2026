...
function matchRenderCharts() {
    try {
        ...
        if(currentMatchTab === 'SAQUE') {
            makeAggregateBarChart('chart-match-agg', pMap, dates, rs => rsPct(rs, 'ACE'), rs => rsEff(rs, 'ACE', 'ERR'), 'Aces (#%)', 'Eficiencia E (%)', 'TOT');
            if(dates.length > 1) {
                makePlayerLineChart('chart-match-pct', pMap, dates, null, r => rPct(r,'ACE'), null, 'Aces (#%)', true, rs => entSum(rs,'TOT')>0);
                makePlayerLineChart('chart-match-eff', pMap, dates, null, r => { var t=r.TOT||0; return t>0?(fr(r.ACE,t)-fr(r.ERR,t))*100:null; }, null, 'Eficiencia E (%)', false, rs => entSum(rs,'TOT')>0);
            } else {
            ...
            makeAggregateBarChart('chart-match-agg', pMap, dates, rs => rsPct(rs, 'POS_PERF') || rsEff(rs,'POS','PERF') + rsPct(rs,'PERF'), rs => rsEff(rs,'POS_PERF','ERR'), '% Acierto (#+)', 'Eficiencia E (%)', 'TOT');
            if(dates.length > 1) {
                makePlayerLineChart('chart-match-pct', pMap, dates, null, r => { var pp=r.POS_PERF||(r.POS+r.PERF)||0; return rPct({...r,'_pp':pp},'_pp'); }, null, '% Acierto (#+)', true, rs => entSum(rs,'TOT')>0);
                makePlayerLineChart('chart-match-eff', pMap, dates, null, r => { var t=r.TOT||0; var pp=fr(r.POS_PERF||(r.POS+r.PERF)||0,t); return t>0?(pp-fr(r.ERR,t))*100:null; }, null, 'Eficiencia E (%)', false, rs => entSum(rs,'TOT')>0);
            } else {
                document.getElementById('match-eff-card').style.display = 'none';
                makeComparisonChart('chart-match-pct', players, [
                    {label:'Total', data:players.map(p=>entSum(pMap[p],'TOT')), backgroundColor:'rgba(0,112,191,.3)'},
            ...
            makeAggregateBarChart('chart-match-agg', pMap, dates, rs => rsPct(rs, 'KILL'), rs => rsEff(rs,'KILL','ERR','BLK'), '% Kills (#%)', 'Eficiencia E (%)', 'TOT');
            if(dates.length > 1) {
                makePlayerLineChart('chart-match-pct', pMap, dates, null, r => rPct(r,'KILL'), null, '% Kills (#%)', true, rs => entSum(rs,'TOT')>0);
                makePlayerLineChart('chart-match-eff', pMap, dates, null, r => { var t=r.TOT||0; return t>0?(fr(r.KILL,t)-fr(r.ERR,t)-fr(r.BLK,t))*100:null; }, null, 'Eficiencia E (%)', false, rs => entSum(rs,'TOT')>0);
            } else {
                document.getElementById('match-eff-card').style.display = 'none';
                makeComparisonChart('chart-match-pct', players, [
                    {label:'Total', data:players.map(p=>entSum(pMap[p],'TOT')), backgroundColor:'rgba(0,112,191,.3)'},
